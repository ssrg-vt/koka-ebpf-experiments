...
// type declarations
/* eBPF changes */
// Need to add this for aliases of uint32_t: 
// typedef __uint32_t uint32_t and typedef unsigned int __uint32_t
#include <inttypes.h> 
...
/* eBPF changes */
// Need to comment the functions that returns bool because first we need to trim the runtime of bool type
...
/* eBPF changes */
/* Remove the boxing and unboxing */
...
static inline kk_eBPF_bpflib_bpf__xdp__md kk_eBPF_bpflib_bpf__new_Xdp__md(kk_reuse_t _at, int32_t _cpath, int32_t data, int32_t data__end, int32_t data__meta, int32_t ingress__ifindex, int32_t rx__queue__index, int32_t egress__ifindex, kk_context_t* _ctx) {
  struct kk_eBPF_bpflib_bpf_Xdp__md* _con = kk_block_alloc_at_as(struct kk_eBPF_bpflib_bpf_Xdp__md, _at, 0, _cpath, (kk_tag_t)(1), _ctx);
  _con->data = data;
